...
{
    public class TriStateTreeViewDelegateRole4Resource : TriStateTreeViewDelegate
    {
        ...
        private string _sqlTextRelation = "SELECT * FROM T_RoleResource where ResourceID={0}";
        private string _sqlText = "";
        private TreeViewExtDataSource<RoleInfo, RoleResource> _tvDataSource;

        internal override void InitTreeViewDataSource()
        {
            Tv.TriStateDelegate = this;
            _tvDataSource = new TreeViewExtDataSource<RoleInfo, RoleResource>();
        ...
        private void AssemblyOldRelation()
        {
            ...
            IList<RoleResource> rrs = (from DataRow row in _dt.Rows select AssemblyRoleResource(row)).ToList();
            _tvDataSource.SetOldRelations(rrs);
        }
        ...
        {
            var ru = new RoleResource { RoleID = ((RoleInfo)tNode.Obj).RoleID, ResourceID = Convert.ToInt32(Tv.RelationOwner),ResourceTypeID = 'W'};
            return ru;
        }
